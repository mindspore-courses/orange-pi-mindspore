...
class CausalSelfAttention(nn.Cell):
    ...
    def construct(self, x):
        ...
        att = (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(k.shape[-1])) # 需要挖空
        # 自注意力计算：
        # 计算查询（Q）和键（K）的点积，得到相似度分数
        # 将分数除以键向量维度的平方根进行归一化操作
...
class GPT(nn.Cell):
            ...
            config.merge_from_dict(
                {
                    ...
                    "gpt-nano":    dict(n_layer=3, n_head=3, n_embd=48), # 需要挖空
                    # demo使用gpt-nano模型，其中n_layer=3, n_head=3, n_embd=48
                }[config.model_type]
            )
    ...
    def construct(self, idx):
        ...
        x = self.transformer.drop(tok_emb + pos_emb) # 挖空
        # x输入应为tok_emb和pos_emb的和，且需要经过dropout处理
        for block in self.h:
            x = block(x)
        x = self.ln_f(x)
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, do_sample=False, top_k=None):
        ...
        for _ in range(max_new_tokens):
            ...
            logits = logits[:, -1, :] / temperature # 挖空
            # 通过在softmax前的logits上除以温度系数，常用于模型校准或生成任务中控制多样性。
            # temperature > 1使分部更平滑（增加不确定性）， temperature < 1使分别更尖锐（增加置信度）
